...
<template>
  ...
  >
    <nav class="p-4">
      ...
      <!-- Divider -->
      <div class="divider my-4">Recent Decks</div>

      <!-- Recent decks -->
      <ul class="menu menu-sm p-0 gap-1">
        <li v-if="decksStore.decks.length === 0">
          <span class="text-base-content/50 italic">No decks yet</span>
        </li>
        <li v-for="deck in decksStore.sortedDecks.slice(0, 5)" :key="deck.id">
          <router-link
            :to="`/decks/${deck.id}`"
      ...
      <div class="mt-6 p-4 bg-base-200 rounded-lg">
        ...
        <div class="flex justify-between text-sm">
          <span class="text-base-content/70">Total Decks</span>
          <span class="font-medium">{{ decksStore.deckCount }}</span>
        </div>
        <div class="flex justify-between text-sm">
          <span class="text-base-content/70">Total Cards</span>
